...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		LOGGER.error("Handling method argement not valid in Process pension microservice");
	...
	public ResponseEntity<ErrorResponse> handleFeignStatusException(FeignException exception,
		...
		LOGGER.error("Handling Feign Client in Process Pension microservice...");
		...
		if (exception.contentUTF8().isBlank()) {
			...
			try {
				...
				LOGGER.debug("Success in parsing the error...");
	...
	public ResponseEntity<ErrorResponse> handleNotFoundException(NotFoundException exception,
			HttpServletResponse response) {
		LOGGER.error("Handling Details mismatch exception in Process Pension microservice");
	...
	public ResponseEntity<ErrorResponse> handleInvalidTokenException(InvalidTokenException exception,
			HttpServletResponse response) {
		LOGGER.error("Handling Invalid Token exception in Process Pension microservice");
		ErrorResponse errorResponse = new ErrorResponse();
		errorResponse.setMessage(exception.getMessage());
		errorResponse.setTimestamp(LocalDateTime.now());
		errorResponse.setFieldErrors(Collections.singletonList(exception.getMessage()));
